...
import { db, pool, accounts, users } from "@workspace/db";
...
async function run(adminEmail: string, adminPassword: string) {
  ...
  await db.transaction(async (tx) => {
    ...
    if (userId) {
      ...
      const inserted = await tx
        .insert(users)
        .values({ name: "Administrador", email: adminEmail, role: "ADMIN", active: true })
