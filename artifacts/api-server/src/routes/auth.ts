...
import { limitLogin } from "../middlewares/rate-limit";

const router: IRouter = Router();
const loginSchema = z.object({ email: z.string().email(), password: z.string().min(8) });

router.post("/v1/auth/login", limitLogin, requireTrustedOrigin, async (req, res) => {
  ...
  if (!existing[0]?.active) {
    req.log.warn({ ip: req.ip }, "Suspicious login attempt");
    return res.status(401).json({ error: "E-mail ou senha inválidos.", requestId: req.id });
  }
